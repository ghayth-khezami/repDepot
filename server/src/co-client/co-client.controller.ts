...
export class CoClientController {
  ...
  async exportCsv(@Res() res: Response) {
    try {
      const result = await this.coClientService.findAll({ limit: 10000, page: 1 });
      const csvData = result.data.map((coClient) => ({
        ...
        'Date Création': new Date(coClient.createdAt).toLocaleDateString('fr-FR'),
      }));
      ...
      res.setHeader('Content-Disposition', 'attachment; filename=co-clients.csv');
      res.send('\ufeff' + csv);
    } catch (error) {
  ...
  async exportPdf(@Res() res: Response) {
    ...
    result.data.forEach((coClient, index) => {
      ...
      x = startX + 2;
      const rowData = [
        coClient.firstName.length > 12 ? coClient.firstName.substring(0, 9) + '...' : coClient.firstName,
        coClient.lastName.length > 12 ? coClient.lastName.substring(0, 9) + '...' : coClient.lastName,
        coClient.email.length > 18 ? coClient.email.substring(0, 15) + '...' : coClient.email,
        coClient.phoneNumber.length > 12 ? coClient.phoneNumber.substring(0, 9) + '...' : coClient.phoneNumber,
        coClient.RIB.length > 12 ? coClient.RIB.substring(0, 9) + '...' : coClient.RIB,
        coClient.address.length > 15 ? coClient.address.substring(0, 12) + '...' : coClient.address,
      ];
