...
export class ClientController {
  ...
  async exportPdf(@Res() res: Response) {
    ...
    result.data.forEach((client, index) => {
      ...
      x = startX + 2;
      const rowData = [
        client.firstName.length > 15 ? client.firstName.substring(0, 12) + '...' : client.firstName,
        client.lastName.length > 15 ? client.lastName.substring(0, 12) + '...' : client.lastName,
        client.email.length > 20 ? client.email.substring(0, 17) + '...' : client.email,
        client.phoneNumber.length > 15 ? client.phoneNumber.substring(0, 12) + '...' : client.phoneNumber,
        client.address.length > 20 ? client.address.substring(0, 17) + '...' : client.address,
      ];
